...
prefix=$3
modeldir=$4       #1120ApplyDM_t1w1.50.out
datadir=$5        #1102MakeRefs.out/fold01
modelext=$6       #dat.post.gz
reffile=$7        #1102MakeRefs.out/fold01.ref
labfile=$8        #0000living.labs
other=$9          #0
cmatidxs=${10}       #1,11-
opt=${11}
...
echo  "eval_activity_cv.E1V1TxMx.sh: $classnum $outdir $modeldir $datadir $modelext $reffile $labfile $other $cmatidxs ($prefix) ($opt) ($dataset)"
...
python get_data_splits.py $outroot/$prefix E1V1Tx $dataset 

for x in `cd $outroot; ls -d ${prefix}*`
do
	echo "Processing $x..."
